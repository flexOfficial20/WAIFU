...
async def harem(update: Update, context: CallbackContext, page=0) -> None:
    ...
    total_pages = math.ceil(len(unique_characters) / 5)  # Now limited to 5 per page
    ...
    for anime, characters in current_grouped_characters.items():
        ...
        for character in characters:
            ...
            rarity = character.get('rarity', '')  # Get rarity from character

            # Only show rarity if it's "Legendary"
            if rarity == "Legendary":
                rarity_display = "🟡 Rarity: Legendary\n"
            else:
                rarity_display = ""  # Don't show anything for other rarities

            harem_message += (
                f"☘️ Name: {escape(character['name'])} (x{count})\n"
                f"{rarity_display}"  # Display rarity only if it's "Legendary"
